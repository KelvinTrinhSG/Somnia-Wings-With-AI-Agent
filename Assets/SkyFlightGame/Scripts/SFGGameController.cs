...
namespace SkyFlightGame
{
	...
	public class SFGGameController : MonoBehaviour 
	{
		...
		{
			if (isGameOver) yield break; // ✅ ngăn gọi lại nếu đã game over
			isGameOver = true;
			...
			if ( gameOverCanvas )    
			{
				...
				float finalScore = (score + PlayerDataManager.Instance.vipNFT * 2) * PlayerDataManager.Instance.ecoFriendly;
